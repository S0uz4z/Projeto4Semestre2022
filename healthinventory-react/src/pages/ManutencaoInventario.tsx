...
export default function ManutencaoInventario() {
  ...
  const { data: items, isLoading, refetch } = useQuery("getItems", getItems, { refetchOnWindowFocus: false});
...
function renderNovoItem(){
  if(editingNewItem == false){
    ...
    novoItemDiv.className = 'flex flex-row list-none bg-[#10543c] h-10 text-white items-center font-bold relative my-1';
    
    let novoItemSpan = document.createElement('span');
    
    let novoItemIconCheck = document.createElement('i');
    novoItemIconCheck.className = 'fa-solid fa-check text-white inline absolute z-50 left-0 top-0 p-3 cursor-pointer'
    ...
    novoItemIconX.className = 'fa-solid fa-xmark text-white inline absolute z-50 left-12 top-0 p-3 cursor-pointer'
    ...
    return data.map((item: any, index: any) => {
      return (
        ...
        >
          ...
          <li className="w-[33%]"><input type={`text`} disabled defaultValue={item.id} className='bg-transparent text-center'></input></li>
          <li className="w-[33%]"><input type={`text`} disabled defaultValue={item.nome} className='bg-transparent text-center'></input></li>
          <li className="w-[33%]"><input type={`text`} disabled defaultValue={item.quantidade} className='bg-transparent text-center'></input></li>
        </div>
      );
    });
}
  ...
  return (
    <div className="w-full h-screen mt-10">
      <div className="w-[50%] min-h-[50%] border-black border-2 border-solid m-auto text-center relative">
        ...
        <div className="flex flex-row absolute bottom-0 p-5"  onClick={()=>{
            renderNovoItem()
          }}>
        ...
        <div className="absolute right-0 bottom-0">
          <Button text="Sair" function={()=>{navigate('/')}}></Button>
        </div>
      </div>
    </div>
  );
